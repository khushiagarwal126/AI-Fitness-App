import { ArrowLeft, PersonStanding, ScaleIcon, User} from "lucide-react"
...
<h1 className="text-2xl font-bold text-slate-800 dark:text-white">FitTrack</h1>
    ...
    <p className="text-slate-500 dark:text-slate-400">Welcome to FitTrack! Let's personalise your fitness journey.</p>
    </div>
    {/*progress Indicator*/}
    ...
    {step === 1 && (
      <div className="space-y-6">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="size-12 rounded-xl bg-emerald-50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-800 flex items-center justify-center">
...
<p className="text-slate-500 dark:text-slate-400 text-sm">This help us calculate your needs</p>
          </div>
      </div>
      <Input 
      label="Age" 
      ...
      className="max-w-2xl"
       value={formData.age}
        onChange={(v)=>updateField('age', v)} placeholder="Enter your age"
         min={13}
          max={120}
           required
           />
...
<p className="text-slate-500 dark:text-slate-400 text-sm">Help us track your progress</p>
      ...
      type="number"
       value={formData.weight}
        onChange={(v)=>updateField('weight', v)} placeholder="Enter your weight"
         min={20}
          max={300}
           required
           />
      ...
      type="number"
       value={formData.height}
        onChange={(v)=>updateField('height', v)} placeholder="Enter your height"
         min={100}
          max={250}
           />
      </div>
...
<p className="text-slate-500 dark:text-slate-400 text-sm">We'll tailer your experience</p>
          </div>
      </div>
      ...
      <div className="space-y-8 max-w-lg">
        ...
        <div className="space-y-6">
          <Slider label="Daily Calorie Intake" min={120} max={4000} step={50} value={formData.dailyCalorieIntake} onChange={(v)=>updateField ('dailyCalorieIntake', v)} unit="calories you plan to consume each day."/>
